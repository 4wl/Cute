    ...
    public void onPacketSend(PacketSendEvent event){
        ...
        if(event.getPacket() instanceof CPacketUseEntity){
            ...
            if(packet.getAction().equals(CPacketUseEntity.Action.ATTACK) && packet.getEntityFromWorld(Wrapper.getWorld()) != null){
                if(packet.getEntityFromWorld(Wrapper.getWorld()) instanceof EntityPlayer && Cute.getInstance().playerStatus.getStatus(packet.getEntityFromWorld(Wrapper.getWorld()).getName()) != 1){
