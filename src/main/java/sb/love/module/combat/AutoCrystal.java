...
import sb.love.util.KillEventHelper;
...
public class AutoCrystal extends Module {
            ...
            if(crystal != null){ // if we found a valid crystal
                ...
                if(tickDelay.getValue() != 0){
                    if(hitCounter >= tickDelay.getValue()){ // check tick delay
                        mc.playerController.attackEntity(Wrapper.getPlayer(), crystal); // attack
                    ...
                    mc.playerController.attackEntity(Wrapper.getPlayer(), crystal);
        ...
        if(placeRange.getValue() > 0){
            ...
            if(!switchCooldown) { // if we're not switching to crystals
                ...
                for (EntityPlayer player : Wrapper.getWorld().playerEntities.stream()
                        ...
                        .filter(p -> Cute.getInstance().playerStatus.getStatus(p.getName()) != 1) // 1 = friend
                        ...
                        .sorted(Comparator.comparing(p -> Cute.getInstance().playerStatus.isEnemyInRange(enemyRange.getValue()) ? Cute.getInstance().playerStatus.getStatus(p.getName()) : Wrapper.getPlayer().getDistance(p)))
    ...
    public void onRender3D(){
        if(espAlpha.getValue() > 0 && renderPos != null && Wrapper.getPlayer() != null && Wrapper.getWorld() != null){
            Color c = Cute.getInstance().clientSettings.getColorr(255);
    ...
    public void onEnable(){
        ...
        if(toggleMsgs.getValue())
            Command.sendClientMessage(getName() + ChatFormatting.GREEN + " LESS GO!", false);
    }
    ...
    public void onDisable(){
        ...
        if(toggleMsgs.getValue())
            Command.sendClientMessage(getName() + ChatFormatting.RED + " GONE", false);
    }
    ...
    private void rotateTo(double x, double y, double z) {
        Cute.getInstance().rotationManager.rotate(x, y, z);
    }
    ...
    private void resetRotation(){
        Cute.getInstance().rotationManager.reset();
    }
