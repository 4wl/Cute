...
import sb.love.Cute;
import sb.love.event.PacketSendEvent;
...
public class KillAura extends Module {
    ...
    public void onUpdate() {
        ...
        if(mode.getValue().equals(Mode.SMART)){
            ...
            if(((AutoCrystal) Cute.getInstance().moduleManager.getModuleByName("AutoCrystal")).isActive) return; // return if AutoCrystal is hitting / placing
        }
        ...
        for(EntityPlayer player : Wrapper.getWorld().playerEntities){
            ...
            if(Cute.getInstance().playerStatus.getStatus(player.getName()) == 1) continue; // ignore if the player is a friend
        ...
        list.sort(Comparator.comparing(p -> Cute.getInstance().playerStatus.isEnemyInRange(range.getValue()) ? Cute.getInstance().playerStatus.getStatus(p.getName()) : Wrapper.getPlayer().getDistance(p)));
    ...
    private void attack(EntityPlayer target) {
        if (Wrapper.getPlayer().getCooledAttackStrength(0f) >= 1f || !delay.getValue()) { // check hit delay
            ...
            if (rotatee) Cute.getInstance().rotationManager.rotate(target.posX, target.posY, target.posZ); // rotate to the target
            ...
            if (rotatee) Cute.getInstance().rotationManager.reset(); // reset rotation
